CLASS z2ui5_cl_demo_app_070_rendra DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
*      END OF ty_s_tab .
    TYPES ty_t_table TYPE STANDARD TABLE OF ztrd_h_001 WITH EMPTY KEY.
...
CLASS z2ui5_cl_demo_app_070_rendra IMPLEMENTATION.
...
*                                                       select   = client->_event( val = `SELKZ` ) )->ui_template( )->checkbox( selected = `{SELKZ}` ).
    lo_columns->ui_column( width                         = '11rem'
                           sortproperty                  = 'HEADREG_UUID'
                                          filterproperty = 'HEADREG_UUID' )->text( text = `HEADREG_UUID` )->ui_template( )->text( text = `{HEADREG_UUID}` ).

    lo_columns->ui_column( width                         = '11rem'
   sortproperty                  = 'REGISTRATION_ID'
   filterproperty = 'REGISTRATION_ID' )->text( text = `REGISTRATION_ID` )->ui_template( )->text( text = `{REGISTRATION_ID}` ).

    lo_columns->ui_column( width                         = '11rem'
sortproperty                  = 'VENDOR_NAME'
filterproperty = 'VENDOR_NAME' )->text( text = `VENDOR_NAME` )->ui_template( )->text( text = `{VENDOR_NAME}` ).



    lo_columns->ui_column( width                         = '11rem'
sortproperty                  = 'VENDOR_CATEGORY'
filterproperty = 'VENDOR_CATEGORY' )->text( text = `VENDOR_CATEGORY` )->ui_template( )->text( text = `{VENDOR_CATEGORY}` ).
*    lo_columns->ui_column( width          = '11rem'
...
*                                                                                                         currency = `{WAERS}` ).
  ...
  METHOD z2ui5_set_data.
        ...
        select * from ztrd_h_001 into table mt_table.
